...
<div class="woocommerce_account_subscriptions">
	...
	<?php if ( ! empty( $subscriptions ) ) : ?>
		<table class="shop_table_responsive my_account_subscriptions table table-vcenter">

			<tr class="active">
				<th colspan="5"><?php esc_html_e( 'My Subscriptions', 'woocommerce-subscriptions' ); ?></th>
			</tr>
		...
		<tr class="order">
			...
			<td class="subscription-status order-status" style="text-align:left; white-space:nowrap;" data-title="<?php esc_attr_e( 'Status', 'woocommerce-subscriptions' ); ?>">
				<?php if ( $subscription->get_status() == 'active' ) : ?>
					<span class="label label-success"><?php echo esc_attr( wcs_get_subscription_status_name( $subscription->get_status() ) ); ?></span>
				<?php else : ?>
					<span class="label label-danger"><?php echo esc_attr( wcs_get_subscription_status_name( $subscription->get_status() ) ); ?></span>
				<?php endif ?>
			</td>
			...
			<td class="subscription-actions order-actions">
				<a href="<?php echo esc_url( $subscription->get_view_order_url() ) ?>" class="view btn btn-default"><?php esc_html_e( 'View', 'woocommerce-subscriptions' ); ?></a>
				<?php if ( $subscription->get_status() == 'active' ) : ?>
					<a class="btn btn-success" href="<?php echo esc_attr( $link ) ?>" class="button view"><?php esc_html_e( 'Sử dụng', NDV_WOO ); ?></a>
				<?php endif; ?>
			</td>
		</tr>
	<?php endforeach; ?>
	...
	<?php else : ?>
		...
		<p class="no_subscriptions"><?php printf( esc_html__( 'Bạn hiện tại không sử dụng dịch vụ nào, xin vui lòng đăng ký tại %sđây%s.', 'woocommerce-subscriptions' ), '<a href="' . esc_url( home_url( 'dich-vu' ) ) . '">', '</a>' ); ?></p>
