...
?>
<table class="subscription_details table table-vcenter">
	<tr>
		<th><?php esc_html_e( 'Status', 'woocommerce-subscriptions' ); ?></th>
		<td>
			<?php if ( $subscription->get_status() == 'active' ) : ?>
				<span class="label label-success"><?php echo esc_html( wcs_get_subscription_status_name( $subscription->get_status() ) ); ?></td></span>
			<?php else : ?>
				<span class="label label-danger"><?php echo esc_html( wcs_get_subscription_status_name( $subscription->get_status() ) ); ?></td></span>
			<?php endif; ?>
	</tr>
	<tr>
		<th><?php esc_html_e( 'Start Date', 'woocommerce-subscriptions' ); ?></th>
...
<table class="order_details table table-vcenter">
	<tbody>
		<tr class="active">
			<th><?php esc_html_e( 'Subscription Totals', 'woocommerce-subscriptions' ); ?></th>
			<td></td>
		</tr>
		<tr>
...
<!--							<td class="remove_item"><a href="--><?php //echo esc_url( WCS_Remove_Item::get_remove_url( $subscription->id, $item_id ) );?><!--" class="remove" onclick="return confirm('--><?php //printf( esc_html__( 'Are you sure you want remove this item from your subscription?', 'woocommerce-subscriptions' ) ); ?>//');">&times;</a></td>
						...
						<td class="product-name">
							...
							if ( $_product && ! $_product->is_visible() ) {
								echo '<b>' . esc_html( apply_filters( 'woocommerce_order_item_name', $item['name'], $item ) ) . '</b>';
							} else {
...
<table class="shop_table_responsive customer_details  table table-vcenter">
	<tr class="active">
		<th><?php esc_html_e( 'Customer details', 'woocommerce-subscriptions' ); ?></th>
		<td></td>
	</tr>
<?php
...
<?php endif; ?>
		...
		<h3 class="h5 font-w600 text-uppercase push-20"><?php esc_html_e( 'Billing Address', 'woocommerce-subscriptions' ); ?></h3>
		<address class="font-s13 push">
			<?php
...
<?php if ( ! wc_ship_to_billing_address_only() && $subscription->needs_shipping_address() && get_option( 'woocommerce_calc_shipping' ) !== 'no' ) : ?>
	...
	<div class="col-2">
		...
		<h3 class="h5 font-w600 text-uppercase push-20"><?php esc_html_e( 'Shipping Address', 'woocommerce-subscriptions' ); ?></h3>
		<address class="font-s13 push">
			<?php
